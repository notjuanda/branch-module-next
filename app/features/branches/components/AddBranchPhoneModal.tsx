...
import { PhoneKind } from "@/api/types";

interface AddBranchPhoneModalProps {
  open: boolean;
  onClose: () => void;
...
export default function AddBranchPhoneModal({
  open,
  onClose,
  branchId,
  onSuccess,
}: AddBranchPhoneModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await branchPhoneService.create(branchId, {
        number: number.trim(),
        kind,
        state: kind === "CORPORATE" ? "AVAILABLE" : undefined,
      ...
      const message = error instanceof Error ? error.message : "Error al agregar el teléfono";
      showError(message);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal
      ...
      subtitle="Nuevo teléfono para la sucursal"
      showCloseButton
      transition="fade"
    >
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5, paddingTop: 2 }}>
        {/* Número */}
        <TextField
        ...
        <Box
          sx={{
            p: 1.5,
            borderRadius: 1,
            backgroundColor: kind === "CORPORATE" 
              ? "rgba(0, 50, 84, 0.05)" 
              : "rgba(34, 197, 94, 0.05)",
            border: `1px solid ${kind === "CORPORATE" ? "var(--color-primary)" : "var(--color-success)"}`,
          }}
        >
        ...
        >
          ...
          <Button
            ...
            startIcon={saving ? <CircularProgress size={16} sx={{ color: "white" }} /> : <HiOutlinePlus size={18} />}
            sx={{
